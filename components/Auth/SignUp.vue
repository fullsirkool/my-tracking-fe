  ...
  <div class="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
    <UForm :schema="schema" :state="state" class="space-y-4" @submit="onSubmit">
      ...
      <UFormGroup label="Confirm Password" name="confirmPassword" size="lg">
        <template #label>
          <label class="text-md font-medium text-gray-900 leading-6">Confirm Password</label>
        </template>
        <UInput v-model="state.confirmPassword" icon="i-heroicons-lock-closed-20-solid" placeholder="Confirm Password"
                :type="isShowConfirmPassword ? 'text' : 'password'" class="custom-input">
          <template #trailing>
      ...
      <div class="flex items-center gap-4">
        <UFormGroup name="firstName" size="lg">
          <template #label>
            <label class="text-md font-medium text-gray-900 leading-6">First Name</label>
          </template>
          <UInput v-model="state.firstName" icon="i-heroicons-user" placeholder="First Name"/>
        </UFormGroup>
        <UFormGroup name="lastName" size="lg">
          <template #label>
            <label class="text-md font-medium text-gray-900 leading-6">Last Name</label>
          </template>
          <UInput v-model="state.lastName" icon="i-heroicons-user" placeholder="Last Name"/>
        </UFormGroup>
      </div>
      <UFormGroup name="sex" size="lg">
        <template #label>
          <label class="text-md font-medium text-gray-900 leading-6">Gender</label>
        </template>
        <USelect
            v-model="state.sex"
        ...
        >
          <template #leading>
            <Icon name="ph:gender-intersex-light" width="1.5rem" height="1.5rem" color="#9ca3af"/>
          </template>
        </USelect>
      </UFormGroup>

<template>
  <div class="relative challenge-card-container" :class="customClass">
    ...
    <UCard class="rounded-3xl bg-[#f5f5f5] challenge-card-content text-[#4B4B4B] max-w-[500px] relative"
      ...
      <div class=" mt-4">
        ...
        <div class="flex items-center justify-start gap-4 ">
          ...
          <div class="w-full">
            <NuxtLink :to="`/challenge/${challenge.id}`" class="font-bold text-xl hover:text-sky-900">{{
                challenge.title
              }}
            </NuxtLink>
            <div class="flex items-center justify-between">
              ...
              <h3 class="mt-1 text-md flex items-center gap-2 cursor-pointer" @click="isOpenShareModal = true">{{
                  $t("share")
                }}
...
const getDisplayDate = computed(() => {
  ...
  return `${dayjs(startDate).format("YYYY, MMM DD")} - ${dayjs(endDate).format("YYYY, MMM DD")}`
})
...
.challenge-card-container {
  .challenge-card-tag, .challenge-card-content {

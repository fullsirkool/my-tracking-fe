...
const validate = (state: any) => {
  ...
  if (!backgroundFile) {
    errors.push({ path: 'background', message: t('thumbnail_is_required_field') })
  }
  return errors
}
...
const submit = async (event: any) => {
  ...
  const backgroundUrl = await uploadImgage(backgroundFile.value)

  const {data, error} = await articleRepository.create({title, content: state.value.description, backgroundUrl})
    ...
    >
      <div class="grid grid-cols-1 md:grid-cols-3">
        ...
        <div class="col-span-1">
          <UFormGroup :label="$t('thumbnail')" name="background" class="p-2" required>
